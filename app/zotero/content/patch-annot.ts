  ...
  const addMenuItem = (
    ...
    for (const props of items) {
      ...
      if (condition(data, getAnnotations)) {
        ...
        let popup = poppers[poppers.length - 1];
        if (!popup) {
          Zotero.log(
        ...
        let menuitem = reader._window.document.createElement("menuitem");
  ...
  let notifierID = null,
    ...
    notifier = {
      ...
      notify(action, type, ids, extraData) {
        ...
        if (!(action === "add" && type === "tab" && readers?.length > 0))
          return;
        revertPatch = around(
          readers[readers.length - 1].constructor.prototype,
          {
