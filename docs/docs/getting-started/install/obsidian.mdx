...
import ManualAvailable from "@site/src/components/manual-check";
...
1. Use link provided above to download `obsidian-zotero-plugin.zip`
  :::note In Case of Error
    If download link above failed to load, you can also find it manually:
    ...
    2. Download the `obsidian-zotero-plugin.zip` from the `Assets` section of the latest release
        - You should find version tagged without `zotero` prefix, something like `0.4.0`
  :::
